...
-(instancetype)initWithBtnPressedByWindowFrame:(CGRect)frame Pressed:(indexPressedBlock)indexPressed{
    ...
    if (self) {
        ...
        [self addTapGesture];
        imgs = @[@"menu_group",@"menu_gruops",@"menu_pro",@"menu_demo"];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(close) name:@"closeMenu" object:nil];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *imgstr = [NSString stringWithFormat:@"%@",imgs[indexPath.row]];
    imgv.image = [UIImage imageNamed:imgstr];
    [cell.contentView addSubview:imgv];
    cell.backgroundColor = [UIColor clearColor];
    UILabel *lb = [[UILabel alloc] initWithFrame:CGRectMake(44, 8, 60, 44-8)];
    lb.text = self.titleList[indexPath.row];
