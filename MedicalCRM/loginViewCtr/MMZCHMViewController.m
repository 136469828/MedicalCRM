...
-(void)createTextFields
{
    ...
    [yzButton setTitleColor: RGB(7, 115, 226) forState:UIControlStateNormal];
...
-(void)next
{
    ...
    if ([phone.text isEqualToString:@""])
    {
        [SVProgressHUD showErrorWithStatus:@"亲,请输入注册手机号码"];
//        [lc showInfoWithStatus:@"亲,请输入注册手机号码"];
        return;
    }
    else if (phone.text.length <11)
    {
        [SVProgressHUD showErrorWithStatus:@"您输入的手机号码格式不正确"];
        return;
    }
