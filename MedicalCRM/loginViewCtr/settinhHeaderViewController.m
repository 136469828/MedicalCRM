...
-(void)createTextFields
{
    ...
    UIButton *landBtn=[self createButtonFrame:CGRectMake(10, bgView.frame.size.height+bgView.frame.origin.y+30,self.view.frame.size.width-20, 37) backImageName:nil title:@"完成" titleColor:[UIColor whiteColor]  font:[UIFont systemFontOfSize:17] target:self action:@selector(clickaddBtn)];
...
//    manger.registerPwd =  self.passward;
//    manger.registerCnName = username.text;
////    NSLog(@"%@ %@ %@",manger.registerName,manger.registerPwd,username.text);
//    [manger loadData:RequestOfregister];
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(pushHome) name:@"register" object:nil];
}
- (void)pushHome
{
    [self.navigationController pushViewController:[[MMZCViewController alloc]init] animated:YES];
