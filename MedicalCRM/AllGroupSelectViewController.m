//
//  AllGroupSelectViewController.m
//  MedicalCRM
//
...
#import "SelctFriendModel.h"
@interface AllGroupSelectViewController ()<UITableViewDataSource,UITableViewDelegate,UISearchControllerDelegate,CNContactPickerDelegate>
{
    NetManger *manger;
    NSArray *titles;
    NSArray *imgs1;
...
@property (nonatomic, strong) NSMutableArray *m_selctFriends;
@property (nonatomic, strong) UITableView *tableView;
@end

@implementation AllGroupSelectViewController
// 销毁通知中心
- (void)dealloc{
...
- (void)viewDidLoad {
    ...
    [_m_selctFriends removeAllObjects];
    if (!_m_selctFriends) {
        _m_selctFriends = [NSMutableArray array];
    }
}
- (void)setDownMenu:(UIButton *)sender
{
    ...
    dropMenu.titleList = @[@"医院拜访",@"创建聊天",@"申报项目",@"申报样机"];
...
- (void)setTableView
{
    ...
    // 分割线
    UIView *line3 = [[UIView alloc] initWithFrame:CGRectMake(0, 35, ScreenWidth , 1)];
...
//    [self.navigationController pushViewController:conversationVC animated:YES];
    SelctFriendModel *model = self.m_dateArray[indexPath.row];
    model.m_chooseBtn = !model.isChooseBtn;
    if (model.m_chooseBtn)
    { //选择了
        [_m_selctFriends addObject:model.UserID];
        
    }
    else
    { //取消

        [_m_selctFriends removeObject:model.UserID];
        
    }
//    [cell rowSelected];
    JCKLog(@"%@",_m_selctFriends);
